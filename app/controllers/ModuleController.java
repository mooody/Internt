 ...
 * ModuleController.
 *
 * Extend all module controllers with this controller. this controller is saved
 * in the db.
 *
 * @author weetech
 */
...
public class ModuleController extends PlanController
{
   ...
   @Before(unless={"index", "peek","setUserAccess","settings","globalSettings","saveUserAccess" })
   public static void checkModuleAuth()
   {
      if(user() instanceof models.SuperAdmin) return;
      
      String controllerName = request.controller;
      String moduleName = controllerName.substring(0, controllerName.indexOf("."));
      Module module = Module.find("byModuleName", moduleName).first();
      List<Module> modules = user().getUserAndGroupModules();

      for (Module m : modules)
      {
         Logger.info("Users modules %s (%s = %s)", m.name, module.id, m.id);
         if (m.id == module.id.longValue())
         {
            return;
         }
      }
      
      String moduleNameTraslated = Messages.get(module.name);
      forbidden(Messages.get("user.has.no.access.to.module", user().name,moduleNameTraslated , user().company.name));
   }

   /**
    * Fixar en länk till indexmetoden för modulen - I index metoden ser du även
    * till och hämtar in aktuella sessionvariabler till modulen.
    *
    * @param moduleId
    */
   ...
   public static void globalSettings(Long moduleId)
   {
      ...
      if (user() instanceof models.SuperAdmin)
      {
         Module module = Module.findById(moduleId);
